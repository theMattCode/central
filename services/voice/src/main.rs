...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "voice_service=info,axum=info,tower_http=info".into()),
        )
        .init();
    ...
    info!(
        ...
        "Loaded voice service configuration"
    );
    ...
    let voice_turn_service = VoiceTurnService::new(
        speech_to_text,
        chat_completion,
        text_to_speech,
    ...
    if let Err(error) = run_http_server(config, voice_turn_service).await {
        error!("{error}");
        std::process::exit(1);
    }
}
...
async fn run_http_server(
    ...
    info!("Starting voice HTTP service on {address}");

    let listener = tokio::net::TcpListener::bind(address)
        .await
        .map_err(|error| format!("Failed to bind voice service socket: {error}"))?;

    axum::serve(listener, app)
        .with_graceful_shutdown(shutdown_signal())
        .await
        .map_err(|error| format!("Voice service HTTP server error: {error}"))?;
