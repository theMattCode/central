...
use crate::{
    context::Context,
    domain::model::VoiceTurnStreamEvent,
    error::AppError,
    http::payload::{
        HealthzResponseDto, StreamErrorEnvelopeDto, StreamErrorPayloadDto,
        VoiceTurnAudioChunkEventDto, VoiceTurnDoneEventDto, VoiceTurnRequestDto,
        VoiceTurnResponseDeltaEventDto, VoiceTurnResponseDto, VoiceTurnTranscriptEventDto,
    },
};
...
pub async fn run_turn(
    ...
    Json(payload): Json<VoiceTurnRequestDto>,
) -> Result<Json<VoiceTurnResponseDto>, AppError> {
    let request = payload.try_into()?;
    let result = context.voice_turn_service.run_turn(request).await?;
...
fn to_stream_event(event: VoiceTurnStreamEvent) -> Event {
    match event {
        VoiceTurnStreamEvent::Transcript { transcript } => {
            serialize_stream_event("transcript", &VoiceTurnTranscriptEventDto { transcript })
        }
        VoiceTurnStreamEvent::ResponseDelta { delta } => {
            serialize_stream_event("response_delta", &VoiceTurnResponseDeltaEventDto { delta })
        }
        VoiceTurnStreamEvent::AudioChunk(audio_chunk) => serialize_stream_event(
            "audio_chunk",
            &VoiceTurnAudioChunkEventDto::from(audio_chunk),
        ),
        VoiceTurnStreamEvent::Done { response_text } => {
            serialize_stream_event("done", &VoiceTurnDoneEventDto { response_text })
        }
    }
}
...
pub async fn run_turn_stream(
    ...
    let request = payload.try_into()?;
    let events = context.voice_turn_service.run_turn_stream(request)?;
