...
impl Config {
    pub fn from_env() -> Result<Self, String> {
        let backend_mode = parse_backend_mode(env::var("VOICE_BACKEND_MODE").ok())?;
        let assistant_name = env::var("VOICE_ASSISTANT_NAME")
            .ok()
        ...
        let config = Self {
            ...
            default_voice_instruction: env::var("VOICE_DEFAULT_VOICE_INSTRUCTION").unwrap_or_else(
                |_| {
                    "Ruhige, warme deutsche Stimme. Sprich klar, freundlich und eher knapp."
                        .to_string()
                },
            ),
            llm_base_url: env::var("VOICE_LLM_BASE_URL")
                .ok()
                .filter(|value| !value.is_empty()),
            llm_api_key: env::var("VOICE_LLM_API_KEY")
                .ok()
                .filter(|value| !value.is_empty()),
            llm_model: env::var("VOICE_LLM_MODEL")
                .ok()
                .filter(|value| !value.is_empty()),
            llm_system_prompt: env::var("VOICE_LLM_SYSTEM_PROMPT")
                .ok()
                .filter(|value| !value.is_empty())
                .unwrap_or_else(|| build_default_llm_system_prompt(&assistant_name)),
            stt_url: env::var("VOICE_STT_URL")
                .ok()
                .filter(|value| !value.is_empty()),
            stt_model: env::var("VOICE_STT_MODEL")
                .ok()
                .filter(|value| !value.is_empty()),
            tts_url: env::var("VOICE_TTS_URL")
                .ok()
                .filter(|value| !value.is_empty()),
            tts_model: env::var("VOICE_TTS_MODEL")
                .ok()
                .filter(|value| !value.is_empty()),
            tts_voice: env::var("VOICE_TTS_VOICE")
                .ok()
                .filter(|value| !value.is_empty())
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if self.backend_mode.uses_json_tts() && self.tts_url.is_none() {
            return Err(format!(
                "VOICE_TTS_URL is required when VOICE_BACKEND_MODE={}",
                self.backend_mode.as_str()
            ));
        }
        ...
        if !self.backend_mode.uses_mock_llm() {
            if self.llm_base_url.is_none() {
                return Err(format!(
                    "VOICE_LLM_BASE_URL is required when VOICE_BACKEND_MODE={}",
                    self.backend_mode.as_str()
                ));
            }
            ...
            if self.llm_model.is_none() {
                return Err(format!(
                    "VOICE_LLM_MODEL is required when VOICE_BACKEND_MODE={}",
                    self.backend_mode.as_str()
                ));
            }
        }
        ...
        if self.backend_mode.requires_llm_api_key() && self.llm_api_key.is_none() {
            return Err(format!(
                "VOICE_LLM_API_KEY is required when VOICE_BACKEND_MODE={}",
                self.backend_mode.as_str()
            ));
        }
        ...
        if self.backend_mode.uses_openai_stt() && self.stt_model.is_none() {
            return Err(format!(
                "VOICE_STT_MODEL is required when VOICE_BACKEND_MODE={}",
                self.backend_mode.as_str()
            ));
        }
        ...
        if self.backend_mode.uses_openai_tts() && self.tts_model.is_none() {
            return Err(format!(
                "VOICE_TTS_MODEL is required when VOICE_BACKEND_MODE={}",
                self.backend_mode.as_str()
            ));
        }
...
fn parse_backend_mode(value: Option<String>) -> Result<BackendMode, String> {
    match value.as_deref().unwrap_or("mock") {
        ...
        other => Err(format!(
            "Unsupported VOICE_BACKEND_MODE '{other}'. Expected 'mock', 'llm-proxy', 'openai', or 'proxy'."
        )),
    }
}
...
mod tests {
    ...
    fn llm_proxy_requires_llm_base_url() {
        ...
        assert_eq!(
            config.validate(),
            Err("VOICE_LLM_BASE_URL is required when VOICE_BACKEND_MODE=llm-proxy".to_string())
        );
    }
    ...
    fn proxy_still_requires_stt_and_tts() {
        ...
        assert_eq!(
            config.validate(),
            Err("VOICE_STT_URL is required when VOICE_BACKEND_MODE=proxy".to_string())
        );
        ...
        assert_eq!(
            config.validate(),
            Err("VOICE_TTS_URL is required when VOICE_BACKEND_MODE=proxy".to_string())
        );
    }
    ...
    fn openai_requires_api_key() {
        ...
        assert_eq!(
            config.validate(),
            Err("VOICE_LLM_API_KEY is required when VOICE_BACKEND_MODE=openai".to_string())
        );
    }
    ...
    fn openai_requires_stt_and_tts_models() {
        ...
        assert_eq!(
            config.validate(),
            Err("VOICE_STT_MODEL is required when VOICE_BACKEND_MODE=openai".to_string())
        );
        ...
        assert_eq!(
            config.validate(),
            Err("VOICE_TTS_MODEL is required when VOICE_BACKEND_MODE=openai".to_string())
        );
    }
