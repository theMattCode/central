...
use crate::{
    domain::model::{
        ChatCompletion, ChatTurnRequest, SpeechToText, SynthesisRequest, TextToSpeech,
        TranscriptionRequest, VoiceTurnAudioChunk, VoiceTurnRequest, VoiceTurnResult,
        VoiceTurnStreamEvent,
    },
    error::AppError,
};
...
async fn send_stream_event(
    event_sender: &mpsc::Sender<Result<VoiceTurnStreamEvent, AppError>>,
    event: VoiceTurnStreamEvent,
) -> bool {
...
impl VoiceTurnService {
    ...
    fn resolve_turn_settings(&self, request: &VoiceTurnRequest) -> (String, String) {
    ...
    fn validate_request(&self, request: &VoiceTurnRequest) -> Result<(), AppError> {
        if request.audio_bytes.is_empty() {
            return Err(AppError::BadRequest(
                "Voice turn payload did not contain audio bytes.".to_string(),
            ));
        }
    ...
    pub async fn run_turn(&self, request: VoiceTurnRequest) -> Result<VoiceTurnResult, AppError> {
    ...
    ) -> Result<ReceiverStream<Result<VoiceTurnStreamEvent, AppError>>, AppError> {
        self.validate_request(&request)?;
    ...
    async fn run_turn_stream_task(
        ...
        if !send_stream_event(
            &event_sender,
            VoiceTurnStreamEvent::Transcript {
                transcript: transcript.clone(),
            },
        )
        .await
        ...
        let tts_worker = tokio::spawn(async move {
            while let Some((chunk_index, text)) = tts_receiver.recv().await {
                ...
                if !send_stream_event(
                    ...
                    VoiceTurnStreamEvent::AudioChunk(VoiceTurnAudioChunk {
                        chunk_index,
                        text,
                        audio_bytes: synthesis.audio_bytes,
                        audio_mime_type: synthesis.audio_mime_type,
        ...
        while let Some(delta_result) = delta_stream.next().await {
            ...
            if !send_stream_event(
                &event_sender,
                VoiceTurnStreamEvent::ResponseDelta {
                    delta: delta.clone(),
                },
            )
            .await
        ...
        let _ =
            send_stream_event(&event_sender, VoiceTurnStreamEvent::Done { response_text }).await;
    }
}
