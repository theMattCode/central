...
use crate::{
    ...
    domains::{
        finance::domain::{
            ...
            model::{
                format_amount_minor_units, summarize, MonthQuery, TransactionDraft,
...
impl FinanceDataStore for FakeFinanceStore {
    async fn list_transactions(
        ...
        let transactions = self
            ...
            .iter()
            .filter(|transaction| {
                transaction.transaction_date >= month.start_inclusive
...
fn test_context() -> Context {
    Context {
        ...
        weather_service: WeatherSnapshotService::new(
...
async fn create_and_list_transactions_returns_summary() {
    ...
    let list_response = client
        .get(format!(
            "{base_url}/api/v1/finance/transactions?month=2026-05"
        ))
        .send()
        .await
