...
use crate::{
    domains::finance::domain::{
        contracts::FinanceDataStore,
        model::{
            format_amount_minor_units, summarize, MonthQuery, TransactionDirection,
            TransactionDraft, TransactionListResponse, TransactionResponse,
        },
    },
    error::ApiError,
};
    ...
    async fn list_transactions(
        ...
        let rows = self
            ...
            .query(
                ...
                &[&month.start_inclusive, &month.end_exclusive],
            )
            .await
            .map_err(|error| {
                ApiError::Internal(format!(
                    "Failed to list finance transactions for month {}: {error}",
                    month.month
                ))
            })?;
        ...
        Ok(summarize(month.month.clone(), transactions))
    }
