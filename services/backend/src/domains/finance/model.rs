...
impl TransactionsQueryInput {
  pub fn into_transactions_query(self) -> Result<TransactionsQuery, ApiError> {
    let from = required_iso_date(self.from, "from")?;
    let to = required_iso_date(self.to, "to")?;

    if to < from {
      return Err(ApiError::BadRequest(
        "Query parameter to must not be before from".to_string(),
      ));
    }
