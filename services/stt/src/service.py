...
def load_config() -> Config:
    return Config(
        port=parse_int_env("STT_PORT", 8081),
        model=os.getenv("STT_MODEL", "small").strip() or "small",
        device=os.getenv("STT_DEVICE", "cpu").strip() or "cpu",
        compute_type=os.getenv("STT_COMPUTE_TYPE", "int8").strip() or "int8",
        beam_size=parse_int_env("STT_BEAM_SIZE", 5),
        vad_filter=parse_bool_env("STT_VAD_FILTER", True),
    )
...
def run() -> None:
    logging.basicConfig(
        level=os.getenv("STT_LOG_LEVEL", "INFO").upper(),
    ...
    LOGGER.info(
        "Starting STT service on port=%s model=%s device=%s compute_type=%s",
        config.port,
        config.model,
        config.device,
        config.compute_type,
