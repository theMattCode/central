...
def load_config() -> Config:
    return Config(
        port=parse_int_env("LLM_PORT", 8083),
        ollama_url=os.getenv("LLM_OLLAMA_URL", "http://service-llm-runtime:11434")
        .strip()
        .rstrip("/"),
        request_timeout_seconds=parse_int_env("LLM_REQUEST_TIMEOUT_SECONDS", 120),
...
def build_openai_chat_response(model: str, content: str) -> dict[str, object]:
    return {
        "id": f"chatcmpl-central-{int(time.time())}",
...
def ensure_model_available(config: Config, model: str) -> None:
    with MODEL_PULL_LOCK:
        ...
        LOGGER.info("Pulling Ollama model '%s' if needed", model)
        post_json(
...
def run() -> None:
    ...
    LOGGER.info(
        "Starting LLM service on port=%s ollama_url=%s default_model=%s",
        config.port,
        config.ollama_url,
