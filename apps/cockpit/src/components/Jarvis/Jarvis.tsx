...
import { formatJarvisPercent, resolveJarvisSystemState, type JarvisMode } from '@/components/Jarvis/model.ts';
import { configureVoiceOrt, VAD_BASE_ASSET_PATH } from '@/widgets/voice/model/vadAssetPaths.ts';
import { getFloat32SignalLevel } from '@/widgets/voice/model/audioLevel.ts';
import { useVoiceConversation } from '@/widgets/voice/model/useVoiceConversation.ts';
...
function JarvisOrb({ bars, energy, mode }: JarvisOrbProps) {
...
export function Jarvis() {
  ...
  return (
    <section className="relative" data-mode={systemState.mode} id="jarvis">
      <div className="jarvis-shell__scanline" aria-hidden="true" />
      ...
      <div className="jarvis-shell__footer">
        <span />
        <span>{systemState.mode.toUpperCase()}</span>
      </div>
