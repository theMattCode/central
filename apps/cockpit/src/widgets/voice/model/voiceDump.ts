...
import type { VoiceTurnInput, VoiceTurnResult, VoiceTurnStreamResult } from './model.ts';

const VOICE_TURN_DUMP_DIRECTORY_NAME = 'tmp';
...
export function createVoiceTurnDumpBaseName(timestamp: Date, turnId: string): string {
  return `${timestamp.toISOString().replaceAll(':', '-').replaceAll('.', '-')}-${turnId}`;
}

export async function dumpVoiceTurnArtifacts(input: VoiceTurnInput, result: VoiceTurnResult): Promise<void> {
  ...
  const dumpDirectory = join(process.cwd(), VOICE_TURN_DUMP_DIRECTORY_NAME);
  ...
  const dumpBaseName = createVoiceTurnDumpBaseName(new Date(), randomUUID());
  ...
  getLogger().info('dumped-voice-turn-artifacts', {
    dumpDirectory,
    inputAudioByteLength: inputBuffer.byteLength,
    inputFilePath,
    metadataFilePath,
...
export async function dumpVoiceTurnStreamArtifacts(
  input: VoiceTurnInput,
  result: VoiceTurnStreamResult,
): Promise<void> {
  if (typeof window !== 'undefined') {
    return;
  }
  ...
  getLogger().info('dumped-streamed-voice-turn-artifacts', {
    audioChunkCount: result.audioChunks.length,
    dumpDirectory,
    inputAudioByteLength: inputBuffer.byteLength,
    inputFilePath,
